...
function spawnWithPrefix(name, cmd, args, opts = {}) {
  console.log(`[start.cjs] spawning -> cmd: ${cmd}, args: ${JSON.stringify(args)}`);
  ...
  p.stderr.on('data', d => {
    ...
    if (/nodist/i.test(s)) {
      console.error('[start.cjs] Detected nodist error. Consider setting a Node version with nodist or install a system Node and set NODE_EXE or FORCE_NODE to its path.');
    }
  });
...
console.log('[start.cjs] nodePath ->', nodePath);
let viteCli = null;
try { viteCli = require.resolve('vite/bin/vite.js'); } catch (e) {
  try { viteCli = require.resolve('vite/cli.js'); } catch (e2) { viteCli = null }
}
console.log('[start.cjs] viteCli ->', viteCli);
// fallback: check node_modules direct path if require.resolve failed
if (!viteCli) {
  const alt = path.resolve(__dirname, '..', 'node_modules', 'vite', 'bin', 'vite.js');
  if (fs.existsSync(alt)) {
    viteCli = alt;
    console.log('[start.cjs] viteCli fallback ->', viteCli);
  }
}
...
serverProc.stdout && serverProc.stdout.on('data', (buf) => {
  ...
  if (s.toLowerCase().includes('listening') || s.toLowerCase().includes('listening on') || s.toLowerCase().includes('ws server listening')) {
    console.log('[start.cjs] server appears to be listening -> starting dev server');
    startDev();
  }
});
